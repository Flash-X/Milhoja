...
void ThreadRoutines::computeLaplacianDensity_cpu(const int tId, Tile& tileDesc) {
    ...
    for     (int j = lo.y; j <= hi.y; ++j) {
        for (int i = lo.x; i <= hi.x; ++i) {
              ...
              f_x_im1 = f(i-1, j,   lo.z, DENS_VAR);
              f_x_ip1 = f(i+1, j,   lo.z, DENS_VAR);
              f_y_im1 = f(i,   j-1, lo.z, DENS_VAR);
              f_y_ip1 = f(i,   j+1, lo.z, DENS_VAR);
              buffer(i, j, lo.z, 0) = 
    ...
    for     (int j = lo.y; j <= hi.y; ++j) {
        for (int i = lo.x; i <= hi.x; ++i) {
            f(i, j, lo.z, DENS_VAR) = buffer(i, j, lo.z, 0);
         }
    } 
}
