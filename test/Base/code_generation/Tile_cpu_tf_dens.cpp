...
void Tile_cpu_tf_dens::acquireScratch(void) {
    ...
    if (base_op1_scratch_) {
        throw std::logic_error("[Tile_cpu_tf_dens::acquireScratch] base_op1_scratch scratch already allocated");
    }
...
void Tile_cpu_tf_dens::releaseScratch(void) {
    if (!base_op1_scratch_) {
        throw std::logic_error("[Tile_cpu_tf_dens::releaseScratch] base_op1_scratch scratch not allocated");
    }
...
#ifdef DEBUG_RUNTIME
    std::string   msg = "[Tile_cpu_tf_dens::releaseScratch] Released base_op1_scratch scratch"
    milhoja::Logger::instance().log(msg);
#endif
}
