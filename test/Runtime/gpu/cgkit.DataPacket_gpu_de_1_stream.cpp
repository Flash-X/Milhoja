...
void DataPacket_gpu_de_1_stream::unpack(void) {
    using namespace milhoja;
    if (tiles_.size() <= 0) throw std::logic_error("[DataPacket_gpu_de_1_stream unpack] Empty data packet.");
    if (!stream_.isValid()) throw std::logic_error("[DataPacket_gpu_de_1_stream unpack] Stream not acquired.");
