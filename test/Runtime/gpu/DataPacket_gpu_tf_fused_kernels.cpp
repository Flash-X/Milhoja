...
std::unique_ptr<milhoja::DataPacket> DataPacket_gpu_tf_fused_kernels::clone(void) const {
    return std::unique_ptr<milhoja::DataPacket>{
        new DataPacket_gpu_tf_fused_kernels {
...
DataPacket_gpu_tf_fused_kernels::DataPacket_gpu_tf_fused_kernels(
...
DataPacket_gpu_tf_fused_kernels::~DataPacket_gpu_tf_fused_kernels(void) {
}


void DataPacket_gpu_tf_fused_kernels::pack(void) {
    using namespace milhoja;
    std::string errMsg = isNull();
    if (errMsg != "")
        throw std::logic_error("[DataPacket_gpu_tf_fused_kernels pack] " + errMsg);
    else if (tiles_.size() == 0)
        throw std::logic_error("[DataPacket_gpu_tf_fused_kernels pack] No tiles added.");
    ...
    if (SIZE_CONSTRUCTOR % ALIGN_SIZE != 0)
        throw std::logic_error("[DataPacket_gpu_tf_fused_kernels pack] SIZE_CONSTRUCTOR padding failure");
    ...
    if (SIZE_TILEMETADATA % ALIGN_SIZE != 0)
        throw std::logic_error("[DataPacket_gpu_tf_fused_kernels pack] SIZE_TILEMETADATA padding failure");
    ...
    if (SIZE_TILEIN % ALIGN_SIZE != 0)
        throw std::logic_error("[DataPacket_gpu_tf_fused_kernels pack] SIZE_TILEIN padding failure");
    ...
    if (SIZE_TILEINOUT % ALIGN_SIZE != 0)
        throw std::logic_error("[DataPacket_gpu_tf_fused_kernels pack] SIZE_TILEINOUT padding failure");
    ...
    if (SIZE_TILEOUT % ALIGN_SIZE != 0)
        throw std::logic_error("[DataPacket_gpu_tf_fused_kernels pack] SIZE_TILEOUT padding failure");
    ...
    if (SIZE_TILESCRATCH % ALIGN_SIZE != 0)
        throw std::logic_error("[DataPacket_gpu_tf_fused_kernels pack] SIZE_TILESCRATCH padding failure");
    ...
    for (auto n = 0; n < _nTiles_h; n++) {
        ...
        if (tileDesc_h == nullptr) throw std::runtime_error("[DataPacket_gpu_tf_fused_kernels pack] Bad tiledesc.");
    ...
    if (!stream_.isValid())
        throw std::runtime_error("[DataPacket_gpu_tf_fused_kernels pack] Unable to acquire stream 1.");
}

void DataPacket_gpu_tf_fused_kernels::unpack(void) {
    using namespace milhoja;
    if (tiles_.size() <= 0)
        throw std::logic_error("[DataPacket_gpu_tf_fused_kernels unpack] Empty data packet.");
    if (!stream_.isValid())
        throw std::logic_error("[DataPacket_gpu_tf_fused_kernels unpack] Stream not acquired.");
