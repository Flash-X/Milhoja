        ...
        for (amrex::MFIter  itor(mfab); itor.isValid(); ++itor) {
            ...
            for     (int j = lo.y; j <= hi.y; ++j) {
                ...
                for (int i = lo.x; i <= hi.x; ++i) {
                    ...
                    data(i, j, lo.z, DENS_VAR) = f1(x, y);
                    data(i, j, lo.z, ENER_VAR) = f2(x, y);
                }
            }
        }
    }
...
//                    absErr = fabs(Delta_f1(x, y) - data(i, j, lo.z, DENS_VAR));
                    absErr = fabs(f1(x, y) - data(i, j, lo.z, DENS_VAR));
                    sum1 += absErr;
...
//                    absErr = fabs(3.2*Delta_f2(x, y) - data(i, j, lo.z, ENER_VAR));
                    absErr = fabs(f2(x, y) - data(i, j, lo.z, ENER_VAR));
                    sum2 += absErr;
...
//    }
    ...
    EXPECT_TRUE(L_inf2 <= 0.0);
    EXPECT_TRUE(0.0 <= meanAbsErr2);
    EXPECT_TRUE(meanAbsErr2 <= 0.0);
}
#endif
