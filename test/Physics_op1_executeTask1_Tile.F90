    ...
    do         k = lo(KAXIS), hi(KAXIS)
        do     j = lo(JAXIS), hi(JAXIS)
            do i = lo(IAXIS), hi(IAXIS)
                u_ij    = Uin(i  , j  , k, DENS_VAR)
                u_im1_j = Uin(i-1, j  , k, DENS_VAR)
                u_ip1_j = Uin(i+1, j  , k, DENS_VAR)
                u_i_jm1 = Uin(i  , j-1, k, DENS_VAR)
                u_i_jp1 = Uin(i  , j+1, k, DENS_VAR)
