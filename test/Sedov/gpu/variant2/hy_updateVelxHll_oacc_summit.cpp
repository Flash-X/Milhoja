...
#if MILHOJA_NDIM == 1
                Uout_d->at(i, j, k, VELX_VAR) = (  Uin_d->at(i,   j, k, VELX_VAR) * Uin_d->at(i, j, k, DENS_VAR)
                                                 + flX_d->at(i,   j, k, HY_XMOM_FLUX)
                                                 - flX_d->at(i+1, j, k, HY_XMOM_FLUX) ) * invNewDens;
#elif MILHOJA_NDIM == 2
                Uout_d->at(i, j, k, VELX_VAR) = (  Uin_d->at(i,   j,   k, VELX_VAR) * Uin_d->at(i, j, k, DENS_VAR)
                                                 + flX_d->at(i,   j,   k, HY_XMOM_FLUX)
                                                 - flX_d->at(i+1, j,   k, HY_XMOM_FLUX)
                                                 + flY_d->at(i,   j,   k, HY_XMOM_FLUX)
                                                 - flY_d->at(i,   j+1, k, HY_XMOM_FLUX) ) * invNewDens;
#elif MILHOJA_NDIM == 3
                Uout_d->at(i, j, k, VELX_VAR) = (  Uin_d->at(i,   j,   k, VELX_VAR) * Uin_d->at(i, j, k, DENS_VAR)
                                                 + flX_d->at(i,   j,   k,   HY_XMOM_FLUX)
                                                 - flX_d->at(i+1, j,   k,   HY_XMOM_FLUX)
                                                 ...
                                                 - flZ_d->at(i,   j,   k+1, HY_XMOM_FLUX) ) * invNewDens;
#endif
            }
        }
    }
