...
DataPacket_Hydro_gpu_2::DataPacket_Hydro_gpu_2(
real dt
...
DataPacket_Hydro_gpu_2::~DataPacket_Hydro_gpu_2(void) {
    if (stream2_.isValid()) throw std::logic_error("[DataPacket_Hydro_gpu_2::~DataPacket_Hydro_gpu_2] Stream 2 not released");
...
int DataPacket_Hydro_gpu_2::extraAsynchronousQueue(const unsigned int id) {
	if((id < 2) || (id > 3 + 1)) throw std::invalid_argument("[DataPacket_Hydro_gpu_2::extraAsynchronousQueue] Invalid id.");
	switch(id) {
		case 2: if(!stream2_.isValid()) { throw std::logic_error("[DataPacket_Hydro_gpu_2::extraAsynchronousQueue] Stream 2 invalid."); } return stream2_.accAsyncQueue;
		case 3: if(!stream3_.isValid()) { throw std::logic_error("[DataPacket_Hydro_gpu_2::extraAsynchronousQueue] Stream 3 invalid."); } return stream3_.accAsyncQueue;
		case 4: if(!stream4_.isValid()) { throw std::logic_error("[DataPacket_Hydro_gpu_2::extraAsynchronousQueue] Stream 4 invalid."); } return stream4_.accAsyncQueue;
	}
	return 0;
}
void DataPacket_Hydro_gpu_2::releaseExtraQueue(const unsigned int id) {
	if((id < 2) || (id > 3 + 1)) throw std::invalid_argument("[DataPacket_Hydro_gpu_2::releaseExtraQueue] Invalid id.");
	switch(id) {
		case 2: if(!stream2_.isValid()) { throw std::logic_error("[DataPacket_Hydro_gpu_2::releaseExtraQueue] Stream 2 invalid."); } milhoja::RuntimeBackend::instance().releaseStream(stream2_); break;
		case 3: if(!stream3_.isValid()) { throw std::logic_error("[DataPacket_Hydro_gpu_2::releaseExtraQueue] Stream 3 invalid."); } milhoja::RuntimeBackend::instance().releaseStream(stream3_); break;
		case 4: if(!stream4_.isValid()) { throw std::logic_error("[DataPacket_Hydro_gpu_2::releaseExtraQueue] Stream 4 invalid."); } milhoja::RuntimeBackend::instance().releaseStream(stream4_); break;
	}
}
...
void DataPacket_Hydro_gpu_2::pack(void) {
    ...
    if (SIZE_CONSTRUCTOR % ALIGN_SIZE != 0)
        throw std::logic_error("[DataPacket_Hydro_gpu_2 pack] SIZE_CONSTRUCTOR padding failure");
    ...
    if (SIZE_TILEMETADATA % ALIGN_SIZE != 0)
        throw std::logic_error("[DataPacket_Hydro_gpu_2 pack] SIZE_TILEMETADATA padding failure");
    ...
    if (SIZE_TILEIN % ALIGN_SIZE != 0)
        throw std::logic_error("[DataPacket_Hydro_gpu_2 pack] SIZE_TILEIN padding failure");
    ...
    if (SIZE_TILEINOUT % ALIGN_SIZE != 0)
        throw std::logic_error("[DataPacket_Hydro_gpu_2 pack] SIZE_TILEINOUT padding failure");
    ...
    if (SIZE_TILEOUT % ALIGN_SIZE != 0)
        throw std::logic_error("[DataPacket_Hydro_gpu_2 pack] SIZE_TILEOUT padding failure");
    ...
    if (SIZE_TILESCRATCH % ALIGN_SIZE != 0)
        throw std::logic_error("[DataPacket_Hydro_gpu_2 pack] SIZE_TILESCRATCH padding failure");
    ...
    if (!stream_.isValid())
        throw std::runtime_error("[DataPacket_Hydro_gpu_2 pack] Unable to acquire stream 1.");
    stream2_ = RuntimeBackend::instance().requestStream(true);
    if(!stream2_.isValid()) throw std::runtime_error("[DataPacket_Hydro_gpu_2::pack] Unable to acquire second stream");
    stream3_ = RuntimeBackend::instance().requestStream(true);
    if(!stream3_.isValid()) throw std::runtime_error("[DataPacket_Hydro_gpu_2::pack] Unable to acquire second stream");
    stream4_ = RuntimeBackend::instance().requestStream(true);
    if(!stream4_.isValid()) throw std::runtime_error("[DataPacket_Hydro_gpu_2::pack] Unable to acquire second stream");
