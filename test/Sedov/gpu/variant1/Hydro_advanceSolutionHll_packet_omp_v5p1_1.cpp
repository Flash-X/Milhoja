...
// See https://www.openmp.org/wp-content/uploads/sc20-booth-talk-interop.pdf
    {
        //----- COMPUTE FLUXES
#       pragma omp target teams distribute nowait default(none) order(concurrent) async(queue_h)
...
#if   NDIM == 1
#       pragma omp target teams distribute default(none) async(queue_h)
        for (std::size_t n=0; n<*nTiles_d; ++n) {
            const PacketContents*  ptrs = contents_d + n;
            const FArray4D*        U_d    = ptrs->CC1_d;
...
#       pragma omp target teams distribute default(none) async(queue2_h)
        for (std::size_t n=0; n<*nTiles_d; ++n) {
            const PacketContents*  ptrs = contents_d + n;
            const FArray4D*        U_d    = ptrs->CC1_d;
            const FArray4D*        auxC_d = ptrs->CC2_d;
...
#       pragma omp target teams distribute default(none) async(queue3_h)
        for (std::size_t n=0; n<*nTiles_d; ++n) {
            const PacketContents*  ptrs = contents_d + n;
            const FArray4D*        U_d    = ptrs->CC1_d;
            const FArray4D*        auxC_d = ptrs->CC2_d;
