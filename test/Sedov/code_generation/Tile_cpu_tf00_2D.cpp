...
void Tile_cpu_tf00_2D::acquireScratch(void) {
    ...
    if (hydro_op1_auxc_) {
        throw std::logic_error("[Tile_cpu_tf00_2D::acquireScratch] hydro_op1_auxc scratch already allocated");
    }
...
void Tile_cpu_tf00_2D::releaseScratch(void) {
    if (!hydro_op1_auxc_) {
        throw std::logic_error("[Tile_cpu_tf00_2D::releaseScratch] hydro_op1_auxc scratch not allocated");
    }
...
#ifdef DEBUG_RUNTIME
    std::string   msg = "[Tile_cpu_tf00_2D::releaseScratch] Released hydro_op1_auxc scratch"
    milhoja::Logger::instance().log(msg);
#endif
}
...
std::unique_ptr<milhoja::TileWrapper> Tile_cpu_tf00_2D::clone(std::unique_ptr<milhoja::Tile>&& tileToWrap) const {
    Tile_cpu_tf00_2D* ptr = new Tile_cpu_tf00_2D{dt_};

    if (ptr->tile_) {
        throw std::logic_error("[Tile_cpu_tf00_2D::clone] Internal tile_ member not null");
    }
    ptr->tile_ = std::move(tileToWrap);
    if (!(ptr->tile_) || tileToWrap) {
        throw std::logic_error("[Tile_cpu_tf00_2D::clone] Wrapper did not take ownership of tile");
    }
