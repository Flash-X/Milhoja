...
void  cpu_tf_IQ_2D::taskFunction(const int threadId,
                    milhoja::DataItem* dataItem) {
    using namespace milhoja;

    Tile_cpu_tf_IQ_2D*  wrapper = dynamic_cast<Tile_cpu_tf_IQ_2D*>(dataItem);
    ...
    milhoja::Real* ptr_mh_internal_volumes =
        static_cast<milhoja::Real*>(Tile_cpu_tf_IQ_2D::_mh_internal_volumes_)
        + Tile_cpu_tf_IQ_2D::_MH_INTERNAL_VOLUMES_SIZE_ * threadId;
