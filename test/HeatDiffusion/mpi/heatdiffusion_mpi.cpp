...
int main(int argc, char* argv[]) {
    ...
    ProcessTimer  heatdiffusion{rp_Simulation::NAME + "_timings.dat", "MPI",
    ...
    while ((nStep <= rp_Simulation::MAX_STEPS) && (Driver::simTime < rp_Simulation::T_MAX)) {
        ...
        //----- UPDATE GRID IF REQUIREDRealVect            deltas   = tileDesc->deltas();
    ...
    if (Driver::simTime >= rp_Simulation::T_MAX) {
        Logger::instance().log("[Simulation] Reached max SimTime");RealVect            deltas   = tileDesc->deltas();
    }
