...
def _generate_cpp2c_helper(data: dict):
    ...
    dpinfo_order = [
        DataPacketMemberVars(item, "", "", False) for item in data[sects.ORDER]
    ]
    ...
    with open("cg-tpl.cpp2c_helper.cpp", "w") as helper:
        ...
        helper.writelines(
            ...
            + [
                ",\n".join(
                    [f"{item[1]} {item[0]}" for item in connectors[_ARG_LIST_KEY]]
                )
            ]
            ...
            + [
                "".join(
                    [
                        f"void* {item.device} = static_cast<void*>( packet_h->{item.device} );\n"
            ...
            + [
                ",".join(
                    [f"{item[1]} {item[0]}" for item in connectors[_INST_ARGS_KEY]]
                )
            ]
            + ["\n\n/* _connector:host_members */\n"]
            + [
                ",".join(
                    [item for item in data.get(sects.GENERAL, {}) if item != "nTiles"]
                )
            ]
        )
