...
def _iterate_constructor(
    connectors: dict, size_connectors: dict, constructor: OrderedDict
):
    """
    ...
    for key, item_type in constructor.items():
        info = DataPacketMemberVars(
            item=key, dtype=item_type, size_eq=f"sizeof({item_type})", per_tile=False
        )
        ...
        connectors[_SET_MEMBERS].extend(
            [f"{info.host}{set_host}", f"{info.device}{{nullptr}}"]
        )
...
def _tmdata_memcpy_f(
    connectors: dict, construct: str, use_ref: str, info: DataPacketMemberVars
):
    """
...
def _iterate_tilemetadata(
    ...
    for item, name in tilemetadata.items():
        ...
        connectors[_SIZE_DET].append(
            f"constexpr std::size_t {info.size} = {size_eq};\n"
        )
        ...
        connectors[_T_DESCRIPTOR].append(f"const auto {name} = tileDesc_h->{name}();\n")
        ...
        if info.dtype in util.F_HOST_EQUIVALENT and language == util.Language.fortran:
            fix_index = (
                "+1" if info.dtype == str("IntVect") else ""
...
def _iterate_lbound(
    connectors: dict, size_connectors: dict, lbound: OrderedDict, lang: str
):
    """
    ...
    lbound_mdata = (
        " + ".join(f"SIZE_{item.upper()}" for item in lbound) if lbound else "0"
    )
    size_connectors[f"size_{jsc.T_MDATA}"] = " + ".join(
        [size_connectors[f"size_{jsc.T_MDATA}"], lbound_mdata]
    )
    for key, bound in lbound.items():
        constructor_expression, memcpy_list = util.format_lbound_string(key, bound)
        info = DataPacketMemberVars(
            item=key, dtype=dtype, size_eq=f"sizeof({dtype_size})", per_tile=True
        )
...
def _iterate_tilein(
    connectors: dict, size_connectors: dict, tilein: OrderedDict, language: str
):
    """
    ...
    for item, data in tilein.items():
        ...
        connectors[_SET_MEMBERS].extend(
            [f"{info.device}{{nullptr}}", f"{info.pinned}{{nullptr}}"]
        )
        connectors[_SIZE_DET].append(
            f"constexpr std::size_t {info.size} = {info.SIZE_EQ};\n"
        )
        ...
        _add_memcpy_connector(
            connectors, jsc.T_IN, extents, item, start, end, info.size, info.dtype
        )
        # temporary measure until the bounds information in JSON is solidified.
        if language == util.Language.cpp:
            cpp_helpers.insert_farray_memcpy(
                connectors, item, "loGC", "hiGC", unks, info.dtype
            )
...
def get_data_pointer_string(item: str):
    ...
    if "flux" in item.lower() or "fl" in item.lower():
        if "X" in item:
            data_pointer_string = util.DATA_POINTER_MAP["flX"]
        elif "Y" in item:
            data_pointer_string = util.DATA_POINTER_MAP["flY"]
        elif "Z" in item:
            data_pointer_string = util.DATA_POINTER_MAP["flZ"]
...
def _add_memcpy_connector(
    ...
    if not start or not end:
        if not start:
            print(
                "WARNING: You are missing a starting index. If this is intentional, ignore this warning."
            )
        if not end:
            print(
                "WARNING: You are missing an ending index. If this is intentional, ignore this warning."
            )
        offset = "0"
        nBytes = size_item
...
def _iterate_tileinout(
    connectors: dict, size_connectors: dict, tileinout: OrderedDict, language: str
) -> None:
    """
    ...
    for item, data in tileinout.items():
        ...
        if language == util.Language.cpp:
            # hardcode loGC and hiGC for now.
            cpp_helpers.insert_farray_memcpy(
                connectors, item, "loGC", "hiGC", unks, info.dtype
            )
    connectors[f"memcpy_{jsc.T_IN_OUT}"].extend(pinnedLocation)
...
def _iterate_tileout(
    connectors: dict, size_connectors: dict, tileout: OrderedDict, language: str
) -> None:
    """
    ...
    for item, data in tileout.items():
        ...
        _add_unpack_connector(
            ...
            info.dtype,
            corresponding_in_data,
            info.ITEM,
        )
        if language == util.Language.cpp:
            cpp_helpers.insert_farray_memcpy(
                connectors, item, "loGC", "hiGC", f"{end} - {start} + 1", info.dtype
            )


def _iterate_tilescratch(
    connectors: dict, size_connectors: dict, tilescratch: OrderedDict, language: str
):
    """
...
def generate_helper_template(data: dict) -> None:
    ...
    with open(data[jsc.HELPERS], "w") as template:
        ...
        tpv[
            "nTiles"
        ] = "int"  # every data packet always needs nTiles so we insert it here.
        tpv = tpv.items()
        _iterate_constructor(connectors, size_connectors, _sort_dict(tpv, sort_func))

        # # SETUP FOR TILE_METADATA
        num_arrays = (
            len(data.get(jsc.T_SCRATCH, {}))
            + len(data.get(jsc.T_IN, {}))
            + len(data.get(jsc.T_IN_OUT, {}))
            + len(data.get(jsc.T_OUT, {}))
        )
        if sizes:
            if lang == util.Language.cpp:
                sort_func = lambda x: sizes.get(util.TILE_VARIABLE_MAPPING[x[1]], 0)
        ...
        for section, funct in {
            jsc.T_IN: _iterate_tilein,
            jsc.T_IN_OUT: _iterate_tileinout,
            jsc.T_OUT: _iterate_tileout,
        }.items():
        ...
        _iterate_tilescratch(
            connectors, size_connectors, _sort_dict(tilescratch, sort_func), lang
        )

        # insert farray variables if necessary.
        if lang == util.Language.cpp:
            cpp_helpers.insert_farray_information(
                data, connectors, _PUB_MEMBERS, _SET_MEMBERS
            )
