...
_APPLICATION_DESCRIPTION = """=== DataPacket Generator ===
...
def main():
    ...
    parser = argparse.ArgumentParser(
        formatter_class=RawTextHelpFormatter, description=_APPLICATION_DESCRIPTION
    )
    parser.add_argument("JSON", help="[mandatory] The JSON file to generate from.")
    parser.add_argument(
        "--language",
        "-l",
        type=consts.Language,
        choices=list(consts.Language),
    ...
    if args.language is None:
        raise _NoLanguageException(
            "You must provide the language of the paired task function!"
        )
    ...
    with open(args.JSON, "r") as file:
        ...
        if args.language == consts.Language.fortran:
            if not data.get(sections.TASK_FUNCTION_NAME, ""):
                raise _NoTaskFunctionNameException(
                    f"Missing {sections.TASK_FUNCTION_NAME}."
                )
