...
class TaskFunctionGenerator_OpenMP_F(AbcCodeGenerator):
    """
    ...
    """
    __LOG_TAG = "Milhoja Fortran/OpenMP Task Function"
    ...
    def __init__(self, tf_spec, indent, logger):
        ...
        super().__init__(
            ...
            TaskFunctionGenerator_OpenMP_F.__LOG_TAG, logger
        )
        ...
        msgs_all = [
            ...
            f"Fortran/OpenMP Module File\t{self.source_filename}",
    ...
    def generate_source_code(self, destination, overwrite):
        ...
        with open(source_filename, "w") as fptr:
            ...
            offloading = []
            # Use in internal subroutines & export for OpenMP
            ...
            for i in range(2, self._tf_spec.n_streams+1):
                queue = f"queue{i}_h"
                fptr.write(f"{INDENT*2}integer(kind=acc_handle_kind), intent(IN) :: {queue} !!! Even for OpenMP !\n")
            ...
            for arg in self._tf_spec.dummy_arguments:
                ...
                if src == EXTERNAL_ARGUMENT:
                    ...
                    fptr.write(f"{INDENT*2}integer, intent(IN), Contiguous :: {arg}_d(:, :)\n")

                elif src == TILE_DELTAS_ARGUMENT:
                    fptr.write(f"{INDENT*2}real, intent(IN), Contiguous :: {arg}_d(:, :)\n")
                    ...
                    fptr.write(f"{INDENT*2}real, intent({intent}), Contiguous :: {arg}_d(:, :, :, :, :)\n")
                    ...
                    array = "(" + ", ".join(tmp) + ")"
                    fptr.write(f"{INDENT*2}{arg_type}, intent(INOUT), Contiguous :: {arg}_d{array}\n")
            ...
            for node in self._tf_spec.internal_subroutine_graph:
                ...
                if opts['MILHOJA_USE_TARGET_ASYNC'] and \
                   (current_queues == ["dataQ_h"]) and \
                        (len(next_queues) > 1):
                    fptr.write(f"{INDENT*2}!$acc wait(dataQ_h)\n")
                    fptr.write(f"{INDENT*2}!$omp taskwait depend(inout:dataQ_h)\n")
                    fptr.write("\n")
                    ...
                    if opts['MILHOJA_USE_TARGET_ASYNC']:
                        ...
                        fptr.write(f"{INDENT*2}!$omp taskwait depend(inout: &\n")
                        for queue in wait_for[:-1]:
                            fptr.write(f"{INDENT*2}!$omp&{INDENT*2}{queue}, &\n")
                        fptr.write(f"{INDENT*2}!$omp&{INDENT*2}{wait_for[-1]} &\n")
                        fptr.write(f"{INDENT*2}!$omp& )\n")
                        fptr.write("\n")
                    ...
                    if opts['MILHOJA_USE_TARGET_ASYNC']:
                        ...
                        fptr.write(f"{INDENT*2}!$omp taskwait depend(inout:( &\n")
                        for queue in current_queues[:-1]:
                            fptr.write(f"{INDENT*2}!$omp& {queue}, &\n")
                        fptr.write(f"{INDENT*2}!$omp& {current_queues[-1]} &\n")
                        fptr.write(f"{INDENT*2}!$omp& )\n")
                        fptr.write("\n")
                else:
                    # Next kernel launch on same stream => no need to wait
                    print(f"else: current_queues is {current_queues}")
                    print(f"else: node is {node}")
                    print(f"else: next_queues is {next_queues}")
                    assert current_queues == ["dataQ_h"]
                    if opts['MILHOJA_USE_TARGET_ASYNC']:
                        assert current_queues == next_queues

                print(f"current_queues was {current_queues}")
                current_queues = next_queues.copy()
                print(f"node is {node}")
                print(f"next_queues is {next_queues}")
                ...
                for subroutine, queue in zip(node, current_queues):
                    ...
                    fptr.write(f"{INDENT*2}!$omp end target teams distribute\n")
                    fptr.write("\n")
            ...
            for idx in range(2, self._tf_spec.n_streams+1):
                ...
                msg = f"Unable to release extra stream / queue {idx} (OpenMP)"
...
#        actual_args = [act for (_, act, _) in arg_triples]
        for _, arg, src in arg_triples:
            ...
            if src == EXTERNAL_ARGUMENT:
                ...
                lines.append(f"{indent*2}integer, target, intent(IN), Contiguous :: {arg}_d(:, :)")

            elif src == TILE_DELTAS_ARGUMENT:
                pointer_extents[arg] = 1
                pointer_types[arg] = "real"
                lines.append(f"{indent*2}real, target, intent(IN), Contiguous :: {arg}_d(:, :)")
                ...
                pointer_types[arg] = "real"
                lines.append(f"{indent*2}real, target, intent({intent}), Contiguous :: {arg}_d(:, :, :, :, :)")
                ...
                pointer_types[arg] = arg_type
                lines.append(f"{indent*2}{arg_type}, target, intent(INOUT), Contiguous :: {arg}_d{array}")
        ...
        for _, arg, src in arg_triples:
            ...
            if ptr_extents > 0:
                ...
                _line = f"{indent*2}{ptr_type}, pointer, Contiguous :: {arg_p}({_ext_str})"
                lines.append(_line)
        lines.append("")
