...
void Tile_cpu_tf_fused::acquireScratch(void) {
    ...
    if (base_op1_scratch_) {
        throw std::logic_error("[Tile_cpu_tf_fused::acquireScratch] base_op1_scratch_ scratch already allocated");
    }
...
void Tile_cpu_tf_fused::releaseScratch(void) {
    if (!base_op1_scratch_) {
        throw std::logic_error("[Tile_cpu_tf_fused::releaseScratch] base_op1_scratch_ scratch not allocated");
    }
...
#ifdef DEBUG_RUNTIME
    std::string   msg = "[Tile_cpu_tf_fused::releaseScratch] Released base_op1_scratch_ scratch"
    milhoja::Logger::instance().log(msg);
#endif
}
