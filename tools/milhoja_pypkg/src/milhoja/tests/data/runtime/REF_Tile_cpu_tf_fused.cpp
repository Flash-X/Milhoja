...
void*  Tile_cpu_tf_fused::base_op1_scratch4D_ = nullptr;

void Tile_cpu_tf_fused::acquireScratch(void) {
    const unsigned int  nThreads = milhoja::Runtime::instance().nMaxThreadsPerTeam();

    if (base_op1_scratch4D_) {
        throw std::logic_error("[Tile_cpu_tf_fused::acquireScratch] base_op1_scratch4D_ scratch already allocated");
    }

    const std::size_t nBytes = nThreads
                    * Tile_cpu_tf_fused::BASE_OP1_SCRATCH4D_SIZE_
                    * sizeof(milhoja::Real);

    milhoja::RuntimeBackend::instance().requestCpuMemory(nBytes, &base_op1_scratch4D_);

#ifdef DEBUG_RUNTIME
    std::string   msg = "[Tile_cpu_tf_fused::acquireScratch] Acquired"
                    + std::to_string(nThreads)
                    + " base_op1_scratch4D_ scratch blocks"
...
void Tile_cpu_tf_fused::releaseScratch(void) {
    if (!base_op1_scratch4D_) {
        throw std::logic_error("[Tile_cpu_tf_fused::releaseScratch] base_op1_scratch4D_ scratch not allocated");
    }

    milhoja::RuntimeBackend::instance().releaseCpuMemory(&base_op1_scratch4D_);
    base_op1_scratch4D_ = nullptr;

#ifdef DEBUG_RUNTIME
    std::string   msg = "[Tile_cpu_tf_fused::releaseScratch] Released base_op1_scratch4D_ scratch"
    milhoja::Logger::instance().log(msg);
#endif
}
