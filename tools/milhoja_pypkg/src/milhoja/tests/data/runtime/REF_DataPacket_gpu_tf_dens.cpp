...
void DataPacket_gpu_tf_dens::pack(void) {
    ...
    for (auto n = 0; n < _nTiles_h; n++) {
        ...
        FArray4D Uin_device{ static_cast<real*>( static_cast<void*>( static_cast<char*>( static_cast<void*>(_Uin_d) ) + n * SIZE_UIN)), tileDesc_h->loGC(), tileDesc_h->hiGC(), 0 + 1 - 0};
        ...
        FArray4D Uout_device{ static_cast<real*>( static_cast<void*>( static_cast<char*>( static_cast<void*>(_Uout_d) ) + n * SIZE_UOUT)), tileDesc_h->loGC(), tileDesc_h->hiGC(), 0 + 1 - 0};
    ...
    if (!stream_.isValid())
        throw std::runtime_error("[DataPacket_gpu_tf_dens pack] Unable to acquire stream 1.");
}

void DataPacket_gpu_tf_dens::unpack(void) {
    using namespace milhoja;
    if (tiles_.size() <= 0)
        throw std::logic_error("[DataPacket_gpu_tf_dens unpack] Empty data packet.");
    if (!stream_.isValid())
        throw std::logic_error("[DataPacket_gpu_tf_dens unpack] Stream not acquired.");
    ...
    for (auto n = 0; n < _nTiles_h; ++n) {
        ...
        constexpr std::size_t nBytes_Uout = (8 + 2 * 1 * MILHOJA_K1D) * (16 + 2 * 1 * MILHOJA_K2D) * (1 + 2 * 1 * MILHOJA_K3D) * ( 0 - 0 + 1 ) * sizeof(real);
