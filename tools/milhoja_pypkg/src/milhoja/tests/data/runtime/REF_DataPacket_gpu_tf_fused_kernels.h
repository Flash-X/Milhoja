#ifndef DATAPACKET_GPU_TF_FUSED_KERNELS_UNIQUE_IFNDEF_H_
#define DATAPACKET_GPU_TF_FUSED_KERNELS_UNIQUE_IFNDEF_H_
...
class DataPacket_gpu_tf_fused_kernels : public milhoja::DataPacket {
public:
    // constructor
    DataPacket_gpu_tf_fused_kernels(
    
    
    );
    // destructor
    ~DataPacket_gpu_tf_fused_kernels(void);

    //helper methods from base DataPacket class.
    std::unique_ptr<milhoja::DataPacket> clone(void) const override;
    DataPacket_gpu_tf_fused_kernels(DataPacket_gpu_tf_fused_kernels&) = delete;
    DataPacket_gpu_tf_fused_kernels(const DataPacket_gpu_tf_fused_kernels&) = delete;
    DataPacket_gpu_tf_fused_kernels(DataPacket_gpu_tf_fused_kernels&& packet) = delete;
    DataPacket_gpu_tf_fused_kernels& operator=(DataPacket_gpu_tf_fused_kernels&) = delete;
    DataPacket_gpu_tf_fused_kernels& operator=(const DataPacket_gpu_tf_fused_kernels&) = delete;
    DataPacket_gpu_tf_fused_kernels& operator=(DataPacket_gpu_tf_fused_kernels&& rhs) = delete;
...
private:
    ...
    static constexpr std::size_t SIZE_UIN = (8 + 2 * 1 * MILHOJA_K1D) * (16 + 2 * 1 * MILHOJA_K2D) * (1 + 2 * 1 * MILHOJA_K3D) * (1 - 0 + 1) * sizeof(real);
    static constexpr std::size_t SIZE_UOUT = (8 + 2 * 1 * MILHOJA_K1D) * (16 + 2 * 1 * MILHOJA_K2D) * (1 + 2 * 1 * MILHOJA_K3D) * ( 1 - 0 + 1 ) * sizeof(real);
