...
module gpu_tf_test2_mod
    ...
    public :: gpu_tf_test2_Fortran
    public :: gpu_tf_test2_Cpp2C

    interface
        !> C++ task function that TimeAdvance passes to Orchestration unit
        subroutine gpu_tf_test2_Cpp2C(C_tId, C_dataItemPtr) &
                bind(c, name="gpu_tf_test2_Cpp2C")
            use iso_c_binding, ONLY : C_PTR
            use milhoja_types_mod, ONLY : MILHOJA_INT
            integer(MILHOJA_INT), intent(IN), value :: C_tId
            type(C_PTR), intent(IN), value :: C_dataItemPtr
        end subroutine gpu_tf_test2_Cpp2C
    end interface

contains

    subroutine gpu_tf_test2_Fortran(         &
                    C_packet_h,      &
                    dataQ_h,         &
                    queue2_h,        &
                    queue3_h,        &
