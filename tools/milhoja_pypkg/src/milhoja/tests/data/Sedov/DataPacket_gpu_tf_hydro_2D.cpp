...
std::unique_ptr<milhoja::DataPacket> DataPacket_gpu_tf_hydro_2D::clone(void) const {
    return std::unique_ptr<milhoja::DataPacket>{
        new DataPacket_gpu_tf_hydro_2D {
            _dt_h
...
DataPacket_gpu_tf_hydro_2D::DataPacket_gpu_tf_hydro_2D(
real dt
...
DataPacket_gpu_tf_hydro_2D::~DataPacket_gpu_tf_hydro_2D(void) {
}


void DataPacket_gpu_tf_hydro_2D::pack(void) {
    using namespace milhoja;
    std::string errMsg = isNull();
    if (errMsg != "")
        throw std::logic_error("[DataPacket_gpu_tf_hydro_2D pack] " + errMsg);
    else if (tiles_.size() == 0)
        throw std::logic_error("[DataPacket_gpu_tf_hydro_2D pack] No tiles added.");
    ...
    if (SIZE_CONSTRUCTOR % ALIGN_SIZE != 0)
        throw std::logic_error("[DataPacket_gpu_tf_hydro_2D pack] SIZE_CONSTRUCTOR padding failure");
    ...
    if (SIZE_TILEMETADATA % ALIGN_SIZE != 0)
        throw std::logic_error("[DataPacket_gpu_tf_hydro_2D pack] SIZE_TILEMETADATA padding failure");
    ...
    if (SIZE_TILEIN % ALIGN_SIZE != 0)
        throw std::logic_error("[DataPacket_gpu_tf_hydro_2D pack] SIZE_TILEIN padding failure");
    ...
    if (SIZE_TILEINOUT % ALIGN_SIZE != 0)
        throw std::logic_error("[DataPacket_gpu_tf_hydro_2D pack] SIZE_TILEINOUT padding failure");
    ...
    if (SIZE_TILEOUT % ALIGN_SIZE != 0)
        throw std::logic_error("[DataPacket_gpu_tf_hydro_2D pack] SIZE_TILEOUT padding failure");
    ...
    if (SIZE_TILESCRATCH % ALIGN_SIZE != 0)
        throw std::logic_error("[DataPacket_gpu_tf_hydro_2D pack] SIZE_TILESCRATCH padding failure");
    ...
    for (auto n = 0; n < _nTiles_h; n++) {
        ...
        if (tileDesc_h == nullptr) throw std::runtime_error("[DataPacket_gpu_tf_hydro_2D pack] Bad tiledesc.");
        ...
        FArray4D U_device{ static_cast<real*>( static_cast<void*>( static_cast<char*>( static_cast<void*>(_U_d) ) + n * SIZE_U)), tileDesc_h->loGC(), tileDesc_h->hiGC(), 8 - 0 + 1};
    ...
    if (!stream_.isValid())
        throw std::runtime_error("[DataPacket_gpu_tf_hydro_2D pack] Unable to acquire stream 1.");
}

void DataPacket_gpu_tf_hydro_2D::unpack(void) {
    using namespace milhoja;
    if (tiles_.size() <= 0)
        throw std::logic_error("[DataPacket_gpu_tf_hydro_2D unpack] Empty data packet.");
    if (!stream_.isValid())
        throw std::logic_error("[DataPacket_gpu_tf_hydro_2D unpack] Stream not acquired.");
