...
class TestTileWrapperGenerator(unittest.TestCase):
    ...
    def testSedovGeneration(self):
        ...
        for task_function in ["IQ"]:
            ...
            ref_src_fname = path.joinpath(f"REF_Tile_cpu_tf_{task_function}.cpp").resolve()
            json_fname_XD = Path(f"cpu_{task_function}.json")
            new_hdr_fname = Path("Tile_cpu_tf_IQ.h")
            new_src_fname = Path("Tile_cpu_tf_IQ.cpp")
            ...
            for dim in [1, 2, 3]:
                ...
                ref_hdr_fname = path.joinpath(f"REF_Tile_cpu_tf_{task_function}_{dim}D.h").resolve()
