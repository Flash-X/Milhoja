...
class TaskFunctionGenerator_OpenACC_F(AbcCodeGenerator):
    ...
    def generate_source_code(self, destination, overwrite):
        ...
        with open(source_filename, "w") as fptr:
            ...
            # Specify Fortran interfaces on C routines that applications will
            # need to use
            fptr.write(f"{INDENT}! NOTE TO MILHOJA USERS:\n")
            fptr.write(f"{INDENT}! The Fortran interfaces defined here should be used to create and manage\n")
            fptr.write(f"{INDENT}! prototype data items to be passed to Milhoja for use with this task\n")
            fptr.write(f"{INDENT}! function.\n")
            fptr.write(f"{INDENT}interface\n")
            fptr.write(f"{INDENT*2}! Instantiate the prototype data packet\n")
            fptr.write(f"{INDENT*2}!\n")
            fptr.write(f"{INDENT*2}! Arguments\n")
            fptr.write(f"{INDENT*2}!\tC_packet - Milhoja-internal handle to the data packet\n")
            fptr.write(f"{INDENT*2}!\tAll others - user-specified external arguments\n")

            name = self._tf_spec.instantiate_packet_C_function
            arg_list = self._tf_spec.constructor_dummy_arguments
            if len(arg_list) == 0:
                fptr.write(f"{INDENT*2}function {name}(C_packet) result(C_ierr) bind(c)\n")
            else:
                fptr.write(f"{INDENT*2}function {name}( &\n")
                fptr.write(f"{INDENT*2}{INDENT*3}C_packet, &\n")
                for arg, _ in arg_list[:-1]:
                    fptr.write(f"{INDENT*2}{INDENT*3}C_{arg}, &\n")
                fptr.write(f"{INDENT*2}{INDENT*3}C_{arg_list[-1][0]} &\n")
                fptr.write(f"{INDENT*2}{INDENT*2}) result(C_ierr) bind(c)\n")
            fptr.write(f"{INDENT*3}use iso_c_binding, ONLY : C_PTR\n")
            fptr.write(f"{INDENT*3}use milhoja_types_mod, ONLY : MILHOJA_INT\n")
            arg_types_all = set([arg_type.lower() for _, arg_type in arg_list])
            if "real" in arg_types_all:
                fptr.write(f"{INDENT*3}use milhoja_types_mod, ONLY : MILHOJA_REAL\n")
            fptr.write(f"{INDENT*3}type(C_PTR), intent(IN) :: C_packet\n")
            for arg, arg_type in arg_list:
                if arg_type.lower() == "real":
                    fptr.write(f"{INDENT*3}real(MILHOJA_REAL), intent(IN), value :: C_{arg}\n")
                else:
                    msg = "No test cases for external args other than real scalars"
                    raise NotImplementedError(msg)
            fptr.write(f"{INDENT*3}integer(MILHOJA_INT) :: C_ierr\n")
            fptr.write(f"{INDENT*2}end function {name}\n")
            fptr.write("\n")

            name = self._tf_spec.delete_packet_C_function
            fptr.write(f"{INDENT*2}! Delete the prototype data packet\n")
            fptr.write(f"{INDENT*2}!\n")
            fptr.write(f"{INDENT*2}! Arguments\n")
            fptr.write(f"{INDENT*2}!{INDENT}C_packet - Milhoja-internal handle obtained when instantiating\n")
            fptr.write(f"{INDENT*2}!{INDENT*4}data packet\n")
            fptr.write(f"{INDENT*2}function {name}(C_packet) result(C_ierr) bind(c)\n")
            fptr.write(f"{INDENT*3}use iso_c_binding, ONLY : C_PTR\n")
            fptr.write(f"{INDENT*3}use milhoja_types_mod, ONLY : MILHOJA_INT\n")
            fptr.write(f"{INDENT*3}type(C_PTR), intent(IN), value :: C_packet\n")
            fptr.write(f"{INDENT*3}integer(MILHOJA_INT) :: C_ierr\n")
            fptr.write(f"{INDENT*2}end function {name}\n")
            fptr.write(f"{INDENT}end interface\n")
            fptr.write("\n")

            # Specify Fortran interfaces on C routines for Milhoja-internal use
            if self._tf_spec.n_streams > 1:
                fptr.write(f"{INDENT}! Milhoja-internal functions.  Milhoja users should not call these.\n")
                fptr.write(f"{INDENT}interface\n")
                name = self._tf_spec.release_stream_C_function
                fptr.write(f"{INDENT*2}function {name}(C_packet, C_id) result(C_ierr) bind(c)\n")
                fptr.write(f"{INDENT*3}use iso_c_binding, ONLY : C_PTR\n")
                fptr.write(f"{INDENT*3}use milhoja_types_mod, ONLY : MILHOJA_INT\n")
                fptr.write(f"{INDENT*3}type(C_PTR), intent(IN), value :: C_packet\n")
                fptr.write(f"{INDENT*3}integer(MILHOJA_INT), intent(IN), value :: C_id\n")
                fptr.write(f"{INDENT*3}integer(MILHOJA_INT) :: C_ierr\n")
                fptr.write(f"{INDENT*2}end function {name}\n")
                fptr.write(f"{INDENT}end interface\n")
                fptr.write("\n")
