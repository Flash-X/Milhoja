...
def check_external_specification(arg, spec, logger):
    """
    If this does not raise an error, then the specification is acceptable.

    At this level, the test cannot know if the name of the associated external
    ...
    if not isinstance(source, str):
        ...
        msg = f"{arg}'s specification not for external argument ({source})"
    ...
    actual = set(spec)
    if actual != expected:
        msg = f"Invalid external specification keys for {arg} ({actual})"
