    ...
    def generate_source_code(self, destination, overwrite):
        ...
        with open(source_filename, "w") as fptr:
            ...
            for arg in sorted(list(scratch_vars)):
                ...
                fptr.write("\n")
                fptr.write("#ifdef DEBUG_RUNTIME\n")
                fptr.write(f'{INDENT}std::string   msg_{arg} = "[{classname}::acquireScratch] Acquired"\n')
                fptr.write(f"{INDENT*5}+ std::to_string(nThreads)\n")
                fptr.write(f'{INDENT*5}+ " {arg}_ scratch blocks";\n')
                fptr.write(f"{INDENT}milhoja::Logger::instance().log(msg_{arg});\n")
                fptr.write("#endif\n")
            fptr.write("}\n")
            fptr.write("\n")
            ...
            for arg in sorted(list(scratch_vars)):
                ...
                fptr.write("\n")
                fptr.write("#ifdef DEBUG_RUNTIME\n")
                fptr.write(f'{INDENT}std::string   msg_{arg} = "[{classname}::releaseScratch] ')
            ...
            fptr.write("{\n")
            fptr.write("#ifdef DEBUG_RUNTIME\n")
            fptr.write(f'{INDENT}std::string   msg_{classname} = "[{classname}] ')
            fptr.write('Creating wrapper object";\n')
            fptr.write(f"{INDENT}milhoja::Logger::instance().log(msg_{classname});\n")
            fptr.write("#endif\n")
            fptr.write("}\n")
            fptr.write("\n")
            ...
            fptr.write("#ifdef DEBUG_RUNTIME\n")
            fptr.write(f'{INDENT}std::string   msg_dest_{classname} = "[~{classname}] ')
            fptr.write('Destroying wrapper object";\n')
            fptr.write(f"{INDENT}milhoja::Logger::instance().log(msg_dest_{classname});\n")
            fptr.write("#endif\n")
            fptr.write("}\n")
            fptr.write("\n")
