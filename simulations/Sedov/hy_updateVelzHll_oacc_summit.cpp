...
#if NDIM == 1
                Uout_d->at(i, j, k, VELZ_VAR_C) = (  Uin_d->at(i,   j, k, VELZ_VAR_C) * Uin_d->at(i, j, k, DENS_VAR_C)
                                                   + flX_d->at(i,   j, k, HY_ZMOM_FLUX_C)
                                                   - flX_d->at(i+1, j, k, HY_ZMOM_FLUX_C) ) * invNewDens;
#elif NDIM == 2
                Uout_d->at(i, j, k, VELZ_VAR_C) = (  Uin_d->at(i,   j,   k, VELZ_VAR_C) * Uin_d->at(i, j, k, DENS_VAR_C)
                                                   + flX_d->at(i,   j,   k, HY_ZMOM_FLUX_C)
                                                   - flX_d->at(i+1, j,   k, HY_ZMOM_FLUX_C)
                                                   + flY_d->at(i,   j,   k, HY_ZMOM_FLUX_C)
                                                   - flY_d->at(i,   j+1, k, HY_ZMOM_FLUX_C) ) * invNewDens;
#elif NDIM == 3
                Uout_d->at(i, j, k, VELZ_VAR_C) = (  Uin_d->at(i,   j,   k,   VELZ_VAR_C) * Uin_d->at(i, j, k, DENS_VAR_C)
                                                   + flX_d->at(i,   j,   k,   HY_ZMOM_FLUX_C)
                                                   - flX_d->at(i+1, j,   k,   HY_ZMOM_FLUX_C)
                                                   ...
                                                   - flZ_d->at(i,   j,   k+1, HY_ZMOM_FLUX_C) ) * invNewDens;
#endif
            }
        }
    }
