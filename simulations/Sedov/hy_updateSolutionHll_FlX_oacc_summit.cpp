    ...
    for         (int k=k_s; k<=k_e; ++k) {
        for     (int j=j_s; j<=j_e; ++j) {
            for (int i=i_s; i<=i_e; ++i) {
                U_d->at(i, j, k, DENS_VAR_C) += (  flX_d->at(i,   j, k, HY_DENS_FLUX_C)
                                                 - flX_d->at(i+1, j, k, HY_DENS_FLUX_C) );
                U_d->at(i, j, k, VELX_VAR_C) += (  flX_d->at(i,   j, k, HY_XMOM_FLUX_C)
                                                 - flX_d->at(i+1, j, k, HY_XMOM_FLUX_C) );
                U_d->at(i, j, k, VELY_VAR_C) += (  flX_d->at(i,   j, k, HY_YMOM_FLUX_C)
                                                 - flX_d->at(i+1, j, k, HY_YMOM_FLUX_C) );
                U_d->at(i, j, k, VELZ_VAR_C) += (  flX_d->at(i,   j, k, HY_ZMOM_FLUX_C)
                                                 - flX_d->at(i+1, j, k, HY_ZMOM_FLUX_C) );
                U_d->at(i, j, k, ENER_VAR_C) += (  flX_d->at(i,   j, k, HY_ENER_FLUX_C)
