...
#include <openacc.h>
#endif
#ifdef MILHOJA_OPENMP_OFFLOADING
#include <omp.h>
#endif

namespace milhoja {

bool   OmpTargetStreamManager::initialized_ = false;
bool   OmpTargetStreamManager::finalized_   = false;
int    OmpTargetStreamManager::nMaxStreams_ = -1;
 ...
 * 
 */
void    OmpTargetStreamManager::finalize(void) {
    if        (!initialized_) {
        throw std::logic_error("[OmpTargetStreamManager::finalize] Never initialized");
    } else if (finalized_) {
        throw std::logic_error("[OmpTargetStreamManager::finalize] Already finalized");
    }

    Logger::instance().log("[OmpTargetStreamManager] Finalizing ...");

    pthread_mutex_lock(&idxMutex_);

    if (streams_.size() != nMaxStreams_) {
        std::string   errMsg =   "[OmpTargetStreamManager::finalize] "
...
#pragma omp interop destroy(targetsync: stream_[i].ompInteropObj)
      streams_[i].ompInteropObj = omp_interop_none; // implied by interop construct
    }
    Logger::instance().log(  "[OmpTargetStreamManager] Destroyed "
                           + std::to_string(streams_.size())
                           + " omp_interop streams");
#elif defined(MILHOJA_OPENACC_OFFLOADING)
    Logger::instance().log(  "[OmpTargetStreamManager] No longer using "
                           + std::to_string(streams_.size())
                           + " omp_target streams / asynchronous queues");
#elif defined(MILHOJA_CUDA_RUNTIME_BACKEND)
    cudaError_t   cErr = cudaErrorInvalidValue;
    for (std::size_t i=0; i<streams_.size(); ++i) {
         cErr = cudaStreamDestroy(streams_[i].cudaStream);
         if (cErr != cudaSuccess) {
            std::string  errMsg = "[OmpTargetStreamManager::finalize] ";
            errMsg += "Unable to destroy OmpTarget stream\n";
            errMsg += "omp_target error - " + std::string(cudaGetErrorName(cErr)) + "\n";
    ...
    Logger::instance().log("[OmpTargetStreamManager] Finalized");
}
 ...
 */
OmpTargetStreamManager&   OmpTargetStreamManager::instance(void) {
    if        (!initialized_) {
        throw std::logic_error("[OmpTargetStreamManager::instance] Singleton not initialized");
    } else if (finalized_) {
        throw std::logic_error("[OmpTargetStreamManager::instance] No access after finalization");
    }

    static OmpTargetStreamManager   manager;
    return manager;
}
 ...
 *
 */
OmpTargetStreamManager::OmpTargetStreamManager(void)
    : streams_{}
{
    Logger::instance().log("[OmpTargetStreamManager] Initializing...");
...
#pragma omp interop init(targetsync: stream.ompInteropObj)
         stream.cudaStream = static_cast<cudaStream_t>(omp_get_interop_property_ptr(stream.ompInteropObj, omp_ipr_targetsync));
         if (stream.cudaStream == nullptr) {
            std::string  errMsg = "[OmpTargetStreamManager::OmpTargetStreamManager] ";
            errMsg += "An expected stream from omp_get_interop_property_ptr() is null\n";
            throw std::runtime_error(errMsg);
         }

         streams_.push_back( std::move(stream) );
    }
    Logger::instance().log(  "[OmpTargetStreamManager] Acquired " 
                           + std::to_string(streams_.size())
                           + " Interop streams from omp_get_interop_property_ptr(..,omp_ipr_targetsync)");
#elif defined(MILHOJA_OPENACC_OFFLOADING)
    Stream         stream{};
    for (int i=0; i<nMaxStreams_; ++i) {
         stream.accAsyncQueue = i + 1;
    ...
    Logger::instance().log(  "[OmpTargetStreamManager] Created "
                           + std::to_string(streams_.size())
                           + " CUDA streams");
#else
    Stream stream{};
    for (int i=0; i<nMaxStreams_; ++i) {
         ...
         if (stream.cudaStream == nullptr) {
            std::string  errMsg = "[OmpTargetStreamManager::OmpTargetStreamManager] ";
            errMsg += "CUDA stream is null\n";
            Logger::instance().log("Wouldbe error: " + errMsg);
	    //            throw std::runtime_error(errMsg);
         }

         streams_.push_back( std::move(stream) );
    }
    Logger::instance().log(  "[OmpTargetStreamManager] Reserved " 
                           + std::to_string(streams_.size())
                           + " stream objects for  asynchronous queues");
#endif

    Logger::instance().log("[OmpTargetStreamManager] Created and ready for use");
...
OmpTargetStreamManager::~OmpTargetStreamManager(void) {
    if (initialized_ && !finalized_) {
        std::cerr << "[OmpTargetStreamManager::~OmpTargetStreamManager] ERROR - Not finalized"
                  << std::endl;
    }
}
 ...
 * \return 
 */
int  OmpTargetStreamManager::numberFreeStreams(void) {
    pthread_mutex_lock(&idxMutex_);
    std::size_t nStreams = streams_.size();
    pthread_mutex_unlock(&idxMutex_);

    if (nStreams > INT_MAX) {
        std::string  errMsg = "[OmpTargetStreamManager::numberFreeStreams] ";
 ...
 */
Stream    OmpTargetStreamManager::requestStream(const bool block) {
    ...
    if (streams_.size() <= 0) {
        if (block) {
            ...
            do {
                Logger::instance().log("[OmpTargetStreamManager] No streams available.  Blocking as requested.");
                pthread_cond_wait(&streamReleased_, &idxMutex_);
//                Logger::instance().log("[OmpTargetStreamManager] Stream has been released");
            } while(streams_.size() <= 0);
        } else {
            Logger::instance().log("[OmpTargetStreamManager] No streams available. Returning null stream as requested.");
            pthread_mutex_unlock(&idxMutex_);

            return Stream{};
        }
    }
    Logger::instance().log("[OmpTargetStreamManager] Stream available, will deliver!");
 ...
 * RuntimeBackend class.
 */
void   OmpTargetStreamManager::releaseStream(Stream& stream) {
#if defined(MILHOJA_CUDA_RUNTIME_BACKEND)||defined(MILHOJA_CUDA_OFFLOADING)
    if (stream.cudaStream == nullptr) {
        throw std::invalid_argument("[OmpTargetStreamManager::releaseStream] "
