...
void ThreadRoutines::computeLaplacian_cpu(const unsigned int tId,
    ...
    for      (int i=lo[IAXIS]; i<=hi[IAXIS]; ++i) {
         for (int j=lo[JAXIS]; j<=hi[JAXIS]; ++j) {
              f_i     = f[i-i0  ][j-j0  ];
              f_x_im1 = f[i-i0-1][j-j0  ];
              f_x_ip1 = f[i-i0+1][j-j0  ];
              f_y_im1 = f[i-i0  ][j-j0-1];
              f_y_ip1 = f[i-i0  ][j-j0+1];
