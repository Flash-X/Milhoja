...
#  ifndef RUNTIME_MUST_USE_TILEITER
    ...
    int   milhoja_runtime_setup_pipeline_cpugpu_c(milhoja::ACTION_ROUTINE pktTaskFunction,
        ...
        if (nThreads < 0) {
            ...
            std::cerr
                << "[milhoja_runtime_setup_pipeline_cpugpu_c] nTilesPerPacket is negative"
                << std::endl;
            return MILHOJA_ERROR_N_TILES_NEGATIVE;
        }
    ...
    int   milhoja_runtime_setup_pipeline_cpugpusplit_c(milhoja::ACTION_ROUTINE pktTaskFunction,
                                                       milhoja::ACTION_ROUTINE tileTaskFunction,
                                                       const int nThreads,
                                                       const int nTilesPerPacket,
        ...
        if (nThreads < 0) {
            std::cerr
                << "[milhoja_runtime_setup_pipeline_cpugpusplit_c] nThreads is negative"
                << std::endl;
            return MILHOJA_ERROR_N_THREADS_NEGATIVE;
        } else if (nTilesPerPacket < 0) {
            std::cerr
                << "[milhoja_runtime_setup_pipeline_cpugpusplit_c] nTilesPerPacket is negative"
                << std::endl;
            return MILHOJA_ERROR_N_TILES_NEGATIVE;
        }
        ...
        try {
            milhoja::Runtime::instance().setupPipelineForCpuGpuSplitTasks("Split CPUGPU Bundle Name",
                    stagger_usec_ui,
                    pktAction,
                    tileAction,
                    *prototype,
            ...
            std::cerr << "[milhoja_runtime_setup_pipeline_cpugpusplit_c] Unknown error caught" << std::endl;
            return MILHOJA_ERROR_UNABLE_TO_SETUP_PIPELINE;
        }
    ...
    int   milhoja_runtime_setup_pipeline_extgpu_c(milhoja::ACTION_ROUTINE taskFunction,
        ...
        if (nThreads < 0) {
            std::cerr
                << "[milhoja_runtime_setup_pipeline_extgpu_c] nThreads is negative"
                << std::endl;
            return MILHOJA_ERROR_N_THREADS_NEGATIVE;
        } else if (nTilesPerPacket < 0) {
            std::cerr
                << "[milhoja_runtime_setup_pipeline_extgpu_c] nTilesPerPacket is negative"
                << std::endl;
            return MILHOJA_ERROR_N_TILES_NEGATIVE;
        }
    ...
    int   milhoja_runtime_teardown_pipeline_gpu_c(const int nThreads,
        ...
        if (nThreads < 0) {      // nThreads: only use in this function
            std::cerr
                << "[milhoja_runtime_teardown_pipeline_gpu_c] nThreads is negative"
                << std::endl;
            return MILHOJA_ERROR_N_THREADS_NEGATIVE;
        } else if (nTilesPerPacket < 0) { // nTilesPerPacket: only use here
            std::cerr
                << "[milhoja_runtime_teardown_pipeline_gpu_c] nTilesPerPacket is negative"
                << std::endl;
            return MILHOJA_ERROR_N_TILES_NEGATIVE;
        }
    ...
    int   milhoja_runtime_teardown_pipeline_cpugpu_c(const int nThreads,
        ...
        if (nThreads < 0) {      // nThreads: only use in this function
            std::cerr
                << "[milhoja_runtime_teardown_pipeline_cpugpu_c] nThreads is negative"
                << std::endl;
            return MILHOJA_ERROR_N_THREADS_NEGATIVE;
        } else if (nTilesPerPacket < 0) { // nTilesPerPacket: only use here
            std::cerr
                << "[milhoja_runtime_teardown_pipeline_cpugpu_c] nTilesPerPacket is negative"
                << std::endl;
            return MILHOJA_ERROR_N_TILES_NEGATIVE;
        }
    ...
    int   milhoja_runtime_teardown_pipeline_cpugpusplit_c(const int nThreads,
                                                          const int nTilesPerPacket) {
        if (nThreads < 0) {      // nThreads: only use in this function
            std::cerr
                << "[milhoja_runtime_teardown_pipeline_cpugpusplit_c] nThreads is negative"
                << std::endl;
            return MILHOJA_ERROR_N_THREADS_NEGATIVE;
        } else if (nTilesPerPacket < 0) { // nTilesPerPacket: only use here
            std::cerr
                << "[milhoja_runtime_teardown_pipeline_cpugpusplit_c] nTilesPerPacket is negative"
                << std::endl;
            return MILHOJA_ERROR_N_TILES_NEGATIVE;
        }

        try {
            milhoja::Runtime::instance().teardownPipelineForCpuGpuSplitTasks(
                    "Split CPUGPU setup Bundle Name");
        } catch (const std::exception& exc) {
            std::cerr << exc.what() << std::endl;
            return MILHOJA_ERROR_UNABLE_TO_TEARDOWN_PIPELINE;
        } catch (...) {
            std::cerr
                << "[milhoja_runtime_teardown_pipeline_cpugpusplit_c] Unknown error caught"
                << std::endl;
            return MILHOJA_ERROR_UNABLE_TO_TEARDOWN_PIPELINE;
        }
    ...
    int   milhoja_runtime_teardown_pipeline_extgpu_c(const int nThreads,
        ...
        if (nThreads < 0) {      // nThreads: only use in this function
            std::cerr
                << "[milhoja_runtime_teardown_pipeline_extgpu_c] nThreads is negative"
                << std::endl;
            return MILHOJA_ERROR_N_THREADS_NEGATIVE;
        } else if (nTilesPerPacket < 0) { // nTilesPerPacket: only use here
            std::cerr
                << "[milhoja_runtime_teardown_pipeline_extgpu_c] nTilesPerPacket is negative"
                << std::endl;
            return MILHOJA_ERROR_N_TILES_NEGATIVE;
        }

        try {
            milhoja::Runtime::instance().teardownPipelineForExtGpuTasks(
                    "Lazy EXTGPU setup Bundle Name");
        } catch (const std::exception& exc) {
            std::cerr << exc.what() << std::endl;
            return MILHOJA_ERROR_UNABLE_TO_TEARDOWN_PIPELINE;
        } catch (...) {
    ...
    int   milhoja_runtime_push_pipeline_cpugpusplit_c(void* tileWrapper,
                                                      void* packet,
                                                      const int nThreads,
                                                      FlashxTileRaw* tileInfo) {
        if (nThreads < 0) {
            std::cerr << "[milhoja_runtime_push_pipeline_cpugpusplit_c] nThreads is negative" << std::endl;
            return MILHOJA_ERROR_N_THREADS_NEGATIVE;
        }
        ...
        try {
            milhoja::Runtime::instance().pushTileToCpuGpuSplitPipeline("Split Bundle Name",
                    *tilePrototype,
                    *prototype, tileInfo->sP, tileInfo->sI, tileInfo->sR);
        } catch (const std::exception& exc) {
            std::cerr << exc.what() << std::endl;
            ...
            std::cerr << "[milhoja_runtime_push_pipeline_cpugpusplit_c] Unknown error caught" << std::endl;
            return MILHOJA_ERROR_UNABLE_TO_EXECUTE_TASKS;
        }
...
#  ifdef RUNTIME_CAN_USE_TILEITER
    ...
    int   milhoja_runtime_execute_tasks_cpugpu_c(milhoja::ACTION_ROUTINE cpuTaskFunction,
        ...
        if        (nDistributorThreads < 0) {
            std::cerr
                << "[milhoja_runtime_execute_tasks_cpugpu_c] nDistributorThreads is negative"
                << std::endl;
            return MILHOJA_ERROR_N_THREADS_NEGATIVE;
        } else if (nThreads < 0) {
            std::cerr
                << "[milhoja_runtime_execute_tasks_cpugpu_c] nThreads is negative"
                << std::endl;
            return MILHOJA_ERROR_N_THREADS_NEGATIVE;
        } else if (nTilesPerPacket < 0) {
            std::cerr
                << "[milhoja_runtime_execute_tasks_cpugpu_c] nTilesPerPacket is negative"
                << std::endl;
            return MILHOJA_ERROR_N_TILES_NEGATIVE;
        }
    ...
    int   milhoja_runtime_execute_tasks_cpugpusplit_c(milhoja::ACTION_ROUTINE cpuTaskFunction,
        ...
        if        (nDistributorThreads < 0) {
            std::cerr
                << "[milhoja_runtime_execute_tasks_cpugpusplit_c] nDistributorThreads is negative"
                << std::endl;
            return MILHOJA_ERROR_N_THREADS_NEGATIVE;
        } else if (nThreads < 0) {
            std::cerr
                << "[milhoja_runtime_execute_tasks_cpugpusplit_c] nThreads is negative"
                << std::endl;
            return MILHOJA_ERROR_N_THREADS_NEGATIVE;
        } else if (nTilesPerPacket < 0) {
            std::cerr
                << "[milhoja_runtime_execute_tasks_cpugpusplit_c] nTilesPerPacket is negative"
                << std::endl;
            return MILHOJA_ERROR_N_TILES_NEGATIVE;
        }
    ...
    int   milhoja_runtime_execute_tasks_extgpu_c(milhoja::ACTION_ROUTINE taskFunction,
        ...
        if        (nDistributorThreads < 0) {
            std::cerr
                << "[milhoja_runtime_execute_tasks_extgpu_c] nDistributorThreads is negative"
                << std::endl;
            return MILHOJA_ERROR_N_THREADS_NEGATIVE;
        } else if (nThreads < 0) {
            std::cerr
                << "[milhoja_runtime_execute_tasks_extgpu_c] nThreads is negative"
                << std::endl;
            return MILHOJA_ERROR_N_THREADS_NEGATIVE;
        } else if (nTilesPerPacket < 0) {
            std::cerr
                << "[milhoja_runtime_execute_tasks_extgpu_c] nTilesPerPacket is negative"
                << std::endl;
            return MILHOJA_ERROR_N_TILES_NEGATIVE;
        }
